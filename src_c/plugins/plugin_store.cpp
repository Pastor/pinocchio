...
namespace Botan {
    ...
    Plugin_Certificate_Store::Plugin_Certificate_Store(std::shared_ptr<SQL_Database> db,
    ...
    Plugin_Certificate_Store::find_cert(const X509_DN &subject_dn, const std::vector<uint8_t> &key_id) const {
        std::shared_ptr<SQL_Database::Statement> stmt;

        const std::vector<uint8_t> dn_encoding = subject_dn.BER_encode();

        if (key_id.empty()) {
            stmt = m_database->new_statement(
    ...
    Plugin_Certificate_Store::find_all_certs(const X509_DN &subject_dn, const std::vector<uint8_t> &key_id) const {
    ...
    std::shared_ptr<const X509_Certificate>
    Plugin_Certificate_Store::find_cert_by_pubkey_sha1(const std::vector<uint8_t> & /*key_hash*/) const {
        throw Not_Implemented("Certificate_Store_In_SQL::find_cert_by_pubkey_sha1");
    }

    std::shared_ptr<const X509_Certificate>
    Plugin_Certificate_Store::find_cert_by_raw_subject_dn_sha256(const std::vector<uint8_t> & /*subject_hash*/) const {
        throw Not_Implemented("Certificate_Store_In_SQL::find_cert_by_raw_subject_dn_sha256");
    }

    std::shared_ptr<const X509_CRL>
    Plugin_Certificate_Store::find_crl_for(const X509_Certificate &subject) const {
        auto all_crls = generate_crls();

        for (const auto& crl: all_crls) {
            if (!crl.get_revoked().empty() && crl.issuer_dn() == subject.issuer_dn())
                return std::make_shared<X509_CRL>(crl);
        }

        return std::shared_ptr<X509_CRL>();
    }

    std::vector<X509_DN> Plugin_Certificate_Store::all_subjects() const {
    ...
    bool Plugin_Certificate_Store::insert_cert(const X509_Certificate &cert) {
    ...
    bool Plugin_Certificate_Store::remove_cert(const X509_Certificate &cert) {
    ...
    std::shared_ptr<const Private_Key> Plugin_Certificate_Store::find_key(const X509_Certificate &cert) const {
        auto stmt = m_database->new_statement("SELECT key FROM " + m_prefix + "keys "
                                                                              "JOIN " + m_prefix + "certificates ON " +
                                              m_prefix + "keys.fingerprint == " + m_prefix +
    ...
    std::vector<std::shared_ptr<const X509_Certificate>>
    Plugin_Certificate_Store::find_certs_for_key(const Private_Key &key) const {
    ...
    bool Plugin_Certificate_Store::insert_key(const X509_Certificate &cert, const Private_Key &key) {
        insert_cert(cert);
    ...
    void Plugin_Certificate_Store::revoke_cert(const X509_Certificate &cert, CRL_Code code, const X509_Time &time) {
        insert_cert(cert);
    ...
    void Plugin_Certificate_Store::affirm_cert(const X509_Certificate &cert) {
    ...
    std::vector<X509_CRL> Plugin_Certificate_Store::generate_crls() const {
        auto stmt = m_database->new_statement(
        ...
        for (const auto& p: crls) {
            ret.emplace_back(p.first, t, t, p.second);
        }
