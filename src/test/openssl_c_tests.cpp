...
/**
Auto free
using BN_af = std::unique_ptr<BIGNUM, decltype(&::BN_free)>;
using RSA_af = std::unique_ptr<RSA, decltype(&::RSA_free)>;
*/
TEST(BaseProvider_C, PKI_Generate) {
	BIGNUM *big = BN_new();
	RSA *rsa = RSA_new();
	int ret;
...

	{
		...
		char *private_key_text = (char *)malloc(private_key_len + 1);
		char *public_key_text = (char *)malloc(public_key_len + 1);
...

	{
...

		{
			...
			size_t x509_len = BIO_pending(x509);
			char *x509_text = (char *)malloc(x509_len + 1);
